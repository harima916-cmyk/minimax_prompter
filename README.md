...
`h3_audio_prompter.py` (単一ファイル版・[A][B] のみ) の UI を踏襲し、
右側のタブに **差し替え [C] / 検証 [D] / モデル比較 / 運用設定** が加わっている。

1. wav を開く → 発話区間が自動検出される (しきい値・無音長は調整可)
2. 台詞を 1 行 1 発話で入力 (「S2: …」で話者指定)
3. 「LLM に渡す固定枠」をコピーして ComfyUI 側の LLM に貼る
4. LLM の出力を「差し替え [C]」タブに貼って実行
   — 個数が食い違うときは対応付けダイアログが開く (無言で捨てない)
5. 「検証 [D]」で機械チェックし、問題がなければ Input Text ノードへ
